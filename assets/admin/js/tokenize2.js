...
(function(factory){
    ...
    Tokenize2.prototype.reorder = function(){
        ...
        if(this.options.sortable){
            ...
            $.each(this.tokensContainer.sortable('toArray', {attribute: 'data-id'}), $.proxy(function(k, v){
    ...
    Tokenize2.prototype.paste = function(){
        ...
        if($pattern.test(this.input.val())){
            $.each(this.input.val().split($pattern), $.proxy(function(_, id){
                this.trigger('tokenize:tokens:add', [id, null, true]);
            }, this))
        }
     ...
     * @returns {Tokenize2}
     */
    Tokenize2.prototype.tokenAdd = function(id, name, force){
        ...
        if($('li.token[data-id="' + id + '"]', this.tokensContainer).length > 0){
            this.trigger('tokenize:tokens:error:duplicate', [id, name]);
            return this;
        }

        if($('option[value="' + id + '"]', this.element).length) {
            $('option[value="' + id + '"]', this.element).attr('selected', 'selected').prop('selected', true);
        } else if(force){
            this.element.append($('<option selected />').val(id).html(name));
        } else if(this.options.tokensAllowCustom){
            this.element.append($('<option selected data-type="custom" />').val(id).html(name));
        } else {
        ...
        $('<li class="token" />')
            ...
            .prepend($('<a class="dismiss" />').on('mousedown touchstart', {}, $.proxy(function(e){
                e.preventDefault();
                this.trigger('tokenize:tokens:remove', [id]);
            }, this)))
    ...
    Tokenize2.prototype.tokenRemove = function(v){
        ...
        $('li.token[data-id="' + v + '"]', this.tokensContainer).remove();
                ...
                case KEYS.BACKSPACE:
                    if(this.input.val().length < 1){
                        ...
                        if($('li.token.pending-delete', this.tokensContainer).length > 0){
                            this.trigger('tokenize:tokens:remove', [$('li.token.pending-delete', this.tokensContainer).first().attr('data-id')]);
                        } else {
                            var $token = $('li.token:last', this.tokensContainer);
                            if($token.length > 0){
                                this.trigger('tokenize:tokens:markForDelete', [$token.attr('data-id')]);
        ...
        $('option', this.element)
            ...
            .each(function(){
                if($regexp.test($this.transliteration($(this).html()))){
                    $items.push({ value: $(this).attr('value'), name: $(this).html() });
                }
            });
        ...
        if(this.isDropdownOpen()){
            var $li = $('<li class="dropdown-item" />').html(this.dropdownItemFormat(item))
                .on('mouseover', $.proxy(function(e){
                    ...
                    e.preventDefault();
                    e.target = this.fixTarget(e.target);
                    this.trigger('tokenize:tokens:add', [$(e.target).attr('data-id'), $(e.target).attr('data-name'), true]);
                }, this));
            if($('li.token[data-id="' + $li.find('a').attr('data-id') + '"]', this.tokensContainer).length < 1){
    ...
    Tokenize2.prototype.dropdownItemFormat = function(item){
        ...
        if(item.hasOwnProperty('name')){
            ...
            if(this.options.searchHighlight){
                ...
                $display = item.name.replace($regex, '<span class="tokenize-highlight">$1</span>');
            } else {
                $display = item.name;
            }
    ...
    Tokenize2.prototype.clear = function(){
        ...
        $.each($('li.token', this.tokensContainer), $.proxy(function(e, item){
            this.trigger('tokenize:tokens:remove', [$(item).attr('data-id')]);
        }, this));
    ...
    Tokenize2.prototype.resetPending = function(){
        ...
        if($token.length > 0){
            this.trigger('tokenize:tokens:cancelDelete', [$token.attr('data-id')]);
     ...
     * @returns {string}
     */
    Tokenize2.prototype.escapeRegex = function(id){
    ...
    /**
     * Retrieve tokens id to an array
     *
     * @returns {Array}
     */
    ...
    Tokenize2.prototype.transliteration = function(name){
        var diacritics = {
            '\u24B6': 'A',
            '\uFF21': 'A',
            '\u00C0': 'A',
